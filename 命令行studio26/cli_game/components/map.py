...
class Char(ABC):
    ...
    def find(self, pos: tuple[int, int] | Literal['w', 'a', 's', 'd']) -> Char | None:
        ...
        if (target[0] < 0) or (target[1] < 0):
            return None
        try:
            return self.map._list[target[0]][target[1]]
        except IndexError:
            return None
    ...
    def change_to(self, new_char: Char, pos=(0, 0), deep_copy=False) -> bool:
        ...
        try:
            ...
            self.map._list[new_char._row][new_char._column] = new_char
            return True
        except IndexError:
            return False
    ...
    def move(self, directory: Literal['w', 'a', 's', 'd'] | Any, leave: type = None, accessible: set | list | tuple = None) -> bool | None:
        ...
        try:
            if self._row + pos[0] < 0 or self._column + pos[1] < 0:
                raise IndexError
            target_pos = self.map._list[self._row + pos[0]][self._column + pos[1]]
            ...
            return False
        self.map._list[self._row][self._column] = (Char.subclasses['Void'] if leave is None else leave)(self._row,
                                                                                                        self._column,
                                                                                                        self.map)
        self._row += pos[0]
        self._column += pos[1]
        self.map._list[self._row][self._column] = self
        return True
...
class Map(Component):
    ...
    def update(self, input):
        for i in [i.copy() for i in self._list]:
            for j in i:
                j.update(input)
